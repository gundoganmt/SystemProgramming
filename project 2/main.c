...
/*
 --> Trying to read /.xdg-volume-info, 0, 0
--> Trying to read /autorun.inf, 0, 0
--> Trying to read /autorun.inf, 0, 0
 */
...
int main( int argc, char *argv[] )
{
	...
	for(i=0;i<NUMBER_OF_ROWS;i++){
		...
		//getchar();
		parse(i,line);
		//fscanf(file,"%s\t%s\t%s\t%s\t%s\t%s",rows[i].code,rows[i].neighborhood,rows[i].city,rows[i].district,rows[i].latitude,rows[i].longitude);
	}
	/*
	for(i=0;i<NUMBER_OF_ROWS;i++){
		if(strcmp(temp_path,rows[i].city) != 0){
